...
st.markdown(f"""
    ...
    div.stButton > button[key*="undo"] {{
        height: 35px !important; font-size: 13px !important; margin-top: 5px !important;
        background: #f8f9fa !important; color: #999 !important; border: 1px solid #eee !important;
    }}
...
# --- 4. 記憶の魔法 ---
s_z = st_javascript("localStorage.getItem('cpa_final_z');")
s_k = st_javascript("localStorage.getItem('cpa_final_k');")
s_m = st_javascript("localStorage.getItem('cpa_final_money');")
...
def save_data():
    st_javascript(f"localStorage.setItem('cpa_final_z', '{st.session_state.z}');")
    st_javascript(f"localStorage.setItem('cpa_final_k', '{st.session_state.k}');")
    st_javascript(f"localStorage.setItem('cpa_final_money', '{st.session_state.money}');")
...
def click(subj, plus=True):
    if plus:
        ...
        st.snow(); st.balloons(); save_data(); time.sleep(2)
    else:
        if subj == "z": st.session_state.z -= 1
        else: st.session_state.k -= 1
        st.session_state.money -= 100
...
st.markdown('</div>', unsafe_allow_html=True) # メインパネル終了
