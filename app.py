...
st.markdown(f"""
    ...
    .stButton > button {{ width: 100% !important; height: 70px !important; font-size: 20px !important; font-weight: bold !important; border-radius: 35px !important; transition: 0.3s; }}
    .stButton > button:hover {{ transform: scale(1.05); }}
    .stButton > button[key*="z_"] {{ background: linear-gradient(135deg, #4FC3F7 0%, #0071BC 100%) !important; color: white !important; }}
    .stButton > button[key*="k_"] {{ background: linear-gradient(135deg, #26C6DA 0%, #00838F 100%) !important; color: white !important; }}
    </style>
...
praises = {
    ...
    "k": ["❄️ 管理会計の天才！意思決定が神がかってる！", "❄️ コスト管理の達人！合格への最短ルートを爆走中！", "❄️ 限界利益も君のやる気もMAXだね！"]
}

# --- 5. メイン表示 ---
st.markdown(f'<div class="top-message">🧸 今日も君は最高に輝いてるよ！</div>', unsafe_allow_html=True)
...
with col_b:
    ...
    st.link_button("🌸 CPA講義ページ", "https://tlp.edulio.com/cpa/mypage/chapter/")
    st.markdown('</div>', unsafe_allow_html=True)

# 🚀 【ここが重要！】クリック処理のド派手アクション
...
with col1:
    ...
    st.subheader("📘 財務会計")
    st.metric("完了数", f"{st.session_state.z} / 70")
...
with col2:
    st.markdown('<div class="pop-card">', unsafe_allow_html=True)
    st.subheader("📙 管理会計")
    st.metric("完了数", f"{st.session_state.k} / 33")
